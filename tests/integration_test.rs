...
#[test]
fn one_task() -> Result<()> {
    let bpmn = Process::new("tests/files/one_task.bpmn")?.task(COUNT_1, func_cnt(1));
...
fn subprocess_nested() -> Result<()> {
    let bpmn = Process::new("tests/files/subprocess_nested.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
    assert_eq!(result.count, 3);
    Ok(())
}
...
#[test]
fn task_fork() -> Result<()> {
    let bpmn = Process::new("tests/files/task_fork.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
    assert_eq!(result.count, 6);
    Ok(())
}
...
fn fork_explosion() -> Result<()> {
    let bpmn = Process::new("tests/files/fork_explosion.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
...
fn parallel_multi() -> Result<()> {
    let bpmn = Process::new("tests/files/parallel_multi.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
    assert_eq!(result.count, 7);
    Ok(())
}
...
fn parallel_join_fork() -> Result<()> {
    let bpmn = Process::new("tests/files/parallel_join_fork.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
    assert_eq!(result.count, 6);
    Ok(())
}
...
fn parallel_parallel_join_fork() -> Result<()> {
    let bpmn =
        Process::new("tests/files/parallel_parallel_join_fork.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
    assert_eq!(result.count, 23);
    Ok(())
}
...
fn parallel_unbalanced() -> Result<()> {
    let bpmn = Process::new("tests/files/parallel_unbalanced.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
    assert_eq!(result.count, 5);
    Ok(())
}
...
fn parallel_unbalanced2() -> Result<()> {
    let bpmn = Process::new("tests/files/parallel_unbalanced2.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
    assert_eq!(result.count, 6);
    Ok(())
}
...
fn parallel_one_in_and_out() -> Result<()> {
    let bpmn = Process::new("tests/files/parallel_one_in_and_out.bpmn")?.task(COUNT_1, func_cnt(1));
    let result = bpmn.run(Counter::default())?;
    assert_eq!(result.count, 1);
    Ok(())
}
